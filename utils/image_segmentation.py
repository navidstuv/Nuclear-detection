...
def random_contrast_adjustment(x):
    if np.random.random() < .3: # Lowering contrast for 30% of times
        min_x, max_x = np.min(x), np.max(x)
        if max_x>min_x:
...
def random_illumination_gradient(x):
    if np.random.random()<0.35:
        ...
        c = np.max(c)-c
        low = np.random.uniform(.5, .7)
        high = np.random.uniform(1., 1.3)
...
def random_sharpness_adjustment(x):
    if np.random.random()<0.7: # Bluring the mage for 40% of times
        sig = np.random.uniform(.6, 1.1) # (.8,1.2)
...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x,weightMap,mask1,mask2,hm, seed=None):
        ...
        if self.channel_contrast_adjustment and not channelShifted:
            if np.random.random() < 0.5:
                x = random_channel_contrast_adjustment(x)
         
        contrasted = False
        if self.contrast_adjustment: #####
        ...
        if self.shear_range and (not isDeformed) and np.random.random()<0.75:
            shear = np.random.uniform(-self.shear_range, self.shear_range)
        else:
            shear = 0

        if (self.zoom_range[0] == 1 and self.zoom_range[1] == 1) or np.random.random()<0.5:
            zx, zy = 1, 1
        else:
        ...
        if transform_matrix is not None:
            ...
            weightMap = apply_transform(weightMap, transform_matrix, img_channel_axis,
                                fill_mode="constant", cval=2.) ### MOSSSIIII ### THIS VALUE MUST HANDEDLED BASED ON THE WEIGHT GENERATION FILE IN MATLAB
...
class NumpyArrayIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        batch_weightMap = np.zeros((len(index_array),) + self.mask_shape, dtype=K.floatx()) #np.zeros(tuple([len(index_array)] + list(self.x.shape)[1:]), dtype=K.floatx())
        ...
        for i, j in enumerate(index_array):
            ...
            weightMap = self.weightMap[j] if self.weightMap is not None else np.zeros_like(mask1)
